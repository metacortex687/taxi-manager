...
Специалист сопровождения не является пользовательской ролью Taxi-manager. Он контролирует техническое состояние приложения с помощью Платформы наблюдаемости и получает email-оповещения при срабатывании правил Grafana.
...
Архитектура Taxi-manager описана набором взаимодополняющих представлений C4. Схемы разделены по назначению: основной функциональный контур, подключаемая подсистема уведомлений менеджеров, стек наблюдаемости и CI/CD-развёртывание.
...
Taxi-manager используется сотрудником автопарка и администратором приложения. Сотрудник автопарка управляет автопарком, просматривает маршруты и получает сформированные отчёты. Разработчик внешних сервисов использует опубликованную OpenAPI-документацию и REST API для создания интеграций. Точки местоположения поступают от внешнего клиента телеметрии. Для обратного геокодирования система обращается к LocationIQ. Встроенный контейнер «Коллектор наблюдаемости» на Grafana Alloy собирает метрики, распределённые трассировки и логи Taxi-manager и передаёт их в Платформу наблюдаемости. Платформа хранит и визуализирует полученные данные, а также формирует оповещения. Специалист сопровождения, CI/CD-контур и подключаемая подсистема уведомлений менеджеров вынесены в отдельные представления. Внутренние контейнеры и варианты физического развёртывания на контекстной диаграмме C1 не показываются.
...
| **Асинхронное Django-приложение** | Длительные SSE-соединения для передачи координат выбранного автомобиля в браузер в режиме реального времени |
...
| **Документация API** | Просмотр автоматически сформированной OpenAPI-схемы Django REST API и статической схемы Высокопроизводительного API |
...
Сотрудник автопарка выбирает автомобиль в веб-интерфейсе, после чего браузер открывает SSE-соединение через Nginx с Асинхронным Django-приложением. Оно получает новые точки автомобиля из PostgreSQL через PgBouncer и передаёт их веб-интерфейсу в виде SSE-событий. Сами точки поступают от Клиента телеметрии: Nginx направляет REST-запросы в Синхронное Django-приложение, которое сохраняет данные в PostgreSQL.
...
<details>
<summary><strong>2. Основная логическая архитектура с подсистемой уведомлений менеджеров</strong></summary>

Это представление дополняет основной функциональный контур подключаемой подсистемой уведомлений менеджеров.

![Основная логическая архитектура с подсистемой уведомлений менеджеров](docs/images/c4/NotificationContainers.png)

Подсистема уведомлений менеджеров получает изменения данных Taxi-manager посредством CDC. Debezium читает изменения из журнала WAL PostgreSQL и публикует исходные CDC-события в Kafka. База данных находится за границей подсистемы уведомлений менеджеров; Debezium не взаимодействует с Django-приложениями.
...
В одном Kafka-кластере используются две группы топиков. Debezium публикует технические события отдельных таблиц в исходные топики `raw_taxi_manager.public.*`. Flink читает их, удаляет избыточные служебные данные и преобразует значимые изменения в события, соответствующие контрактам сервиса уведомлений. Обработанные события публикуются в топики организаций, назначений менеджеров, моделей автомобилей и автомобилей: `taxi_manager.enterprises`, `taxi_manager.assignment_managers`, `taxi_manager.vehicle_models` и `taxi_manager.vehicles`.
...
В локальной базе SQLite сервис хранит UUID и наименования организаций и моделей автомобилей, назначения менеджеров организациям и привязки пользователей Taxi-manager к учётным записям чата. Эти данные используются для выбора получателей и формирования понятного текста сообщения. Состояние чтения сообщений в SQLite не хранится: позиции чтения отслеживает consumer group Kafka. При первоначальной привязке пользователь вводит логин и пароль в чате, после чего сервис выполняет авторизацию через REST API Taxi-manager. Учётные данные используются только для выполнения запроса и не сохраняются. Возможное направление развития — вынести авторизацию из Django-монолита в отдельный сервис.

| Компонент | Назначение |
| --- | --- |
| **CDC-коннектор** | Читает изменения из журнала WAL PostgreSQL и публикует технические CDC-события |
| **Брокер сообщений** | Хранит исходные и обработанные топики одного Kafka-кластера и позиции чтения consumer groups |
| **Потоковая обработка** | Фильтрует и преобразует CDC-события в события для уведомлений |
| **Сервис уведомлений** | Читает обработанные топики Kafka, определяет получателей по организациям, формирует сообщения и отправляет их в чат |
| **Локальное хранилище контекста и привязок** | Хранит организации, модели автомобилей, назначения менеджеров и привязки пользователей к учётным записям чата |
