...
Специалист сопровождения не является пользовательской ролью приложения. Он контролирует техническое состояние приложения с помощью Платформы наблюдаемости и получает email-оповещения при срабатывании правил Grafana.
...
Архитектура приложения описана набором взаимодополняющих представлений C4. В README используются первые два уровня: диаграмма контекста системы (C1) показывает пользователей, программные системы и внешние зависимости, а диаграмма контейнеров (C2) раскрывает отдельно запускаемые приложения и хранилища внутри выбранной системы. Динамические диаграммы дополняют C1 и C2 последовательностями ключевых сценариев, но не вводят новый уровень декомпозиции.
...
<details>
<summary><strong>Словарь C4 по уровням</strong></summary>

#### Уровень C1 — контекст системы

| Термин | Значение в проекте |
| --- | --- |
| **Пользователь или роль (Person)** | Человек, который взаимодействует с программной системой: сотрудник автопарка, администратор, разработчик внешних сервисов или специалист сопровождения |
| **Программная система (Software System)** | Самостоятельная граница ответственности, предоставляющая ценность пользователям или другим системам: Taxi-manager, Система уведомлений менеджеров, Платформа наблюдаемости и CI/CD Taxi-manager |
| **Внешняя программная система (External Software System)** | Тот же элемент Software System, но находящийся за рассматриваемой границей ответственности; это не отдельный тип элемента C4. Примеры: LocationIQ, Почтовый сервис и Чат уведомлений |

#### Уровень C2 — контейнеры

| Термин | Значение в проекте |
| --- | --- |
| **Контейнер C4 (Container)** | Отдельно запускаемое приложение, процесс или хранилище данных с собственной ответственностью и технологией. Контейнер C4 не равен Docker-контейнеру, хотя при конкретном варианте развёртывания они могут совпадать |
| **Связь (Relationship)** | Направленное взаимодействие между элементами; подпись объясняет назначение, а технология уточняет протокол или способ обмена |
| **Граница системы (Software System Boundary)** | Показывает, какие контейнеры принадлежат выбранной программной системе и управляются как единое архитектурное целое |
...
Taxi-manager передаёт эксплуатационную телеметрию в Платформу наблюдаемости. Специалист сопровождения, CI/CD-контур и подключаемая Система уведомлений менеджеров вынесены в отдельные представления. Внутренние контейнеры и варианты физического развёртывания на диаграмме C1 не показываются.

![C1 — диаграмма контекста системы Taxi-manager](docs/images/c4/TaxiManagerSystemContext.png)

### Диаграмма контейнеров (C2)

Основное представление показывает контейнеры C4 и связи между ними. В текущем варианте запуска они в основном соответствуют сервисам Docker Compose, однако это не схема Docker-контейнеров: те же приложения и процессы можно развернуть непосредственно в операционной системе или распределить между узлами.

![C2 — диаграмма контейнеров Taxi-manager](docs/images/c4/TaxiManagerLogicalContainers.png)

| Контейнер C4 | Назначение | Технологии и применение |
| --- | --- | --- |
| **Веб-интерфейс** | Работа сотрудника с автопарком, картами, маршрутами, отчётами и онлайн-обновлениями | React 19 и Mantine формируют интерфейс; TanStack Query работает с серверным состоянием; Leaflet отображает геоданные; Vite собирает приложение |
| **Входной веб-шлюз** | Единая HTTP-точка входа, раздача статических файлов и маршрутизация запросов | Nginx направляет REST и Django Admin в WSGI-приложение, онлайн-отслеживание — в ASGI-приложение, а также публикует веб-интерфейс и документацию |
| **Синхронное Django-приложение** | REST API, аутентификация, контроль доступа, Django Admin, основная бизнес-логика и приём телеметрии | Python 3.12, Django 6, Django REST Framework, GeoDjango и WSGI; `drf-spectacular` формирует OpenAPI-схему |
| **Асинхронное Django-приложение** | Длительные SSE-соединения для передачи координат выбранного автомобиля в браузер | Django ASGI запускается через Gunicorn с Uvicorn Worker; SSE используется для однонаправленных онлайн-обновлений |
| **Пул соединений** | Переиспользование соединений с PostgreSQL и защита от исчерпания лимита подключений | PgBouncer обслуживает прежде всего длительно работающие ASGI-процессы и применяется в расширенных вариантах развёртывания |
| **Фоновый обработчик заданий** | Формирование отчётов и обратное геокодирование вне пользовательского HTTP-запроса | Django management command выполняет задания `django-tasks-db`; PostgreSQL используется как backend очереди |
| **Высокопроизводительный API** | Экспериментальный перенос измеренных узких мест из Django без преждевременной декомпозиции | Rust, Actix Web, Tokio и SQLx; текущий прототип проверяет одиночные и пакетные операции |
| **База данных** | Хранение данных автопарка, поездок, телеметрии, географии, фоновых заданий и результатов геокодирования | PostgreSQL 16 обеспечивает транзакционное хранение, PostGIS 3.5 — географические типы и операции |
| **Документация API** | Интерактивный просмотр контрактов Django REST API и Высокопроизводительного API | Swagger UI загружает сгенерированную Django OpenAPI-схему по HTTP и статическую Rust OpenAPI-схему из read-only bind mount |
| **Коллектор наблюдаемости** | Единая точка сбора и маршрутизации телеметрии приложения | Grafana Alloy получает трассировки по OTLP, собирает метрики через Prometheus scrape, читает журналы контейнеров и отправляет данные в Prometheus, Loki и Tempo |

Документация API загружает схему Django по HTTP. Статическая схема Высокопроизводительного API хранится в файле `rust-api/rust-open-api-schema.yml` и монтируется только для чтения в контейнер Swagger UI по пути `/usr/share/nginx/html/rust-open-api-schema.yml`. Сетевой запрос к Высокопроизводительному API для загрузки этой схемы не выполняется; файловая связь отдельно показана на C4-диаграмме пунктирной линией.
...
![C2 — диаграмма контейнеров системы уведомлений менеджеров](docs/images/c4/NotificationContainers.png)
...
Коллектор наблюдаемости Grafana Alloy является внутренним контейнером приложения и единой точкой маршрутизации данных наблюдаемости. Приложения отправляют ему трассировки по OTLP, Alloy собирает метрики через Prometheus scrape и читает журналы контейнеров, после чего передаёт телеметрию во внешнюю Платформу наблюдаемости.

![C2 — диаграмма контейнеров наблюдаемости Taxi-manager](docs/images/c4/ObservabilityContainers.png)

Платформа наблюдаемости хранит и предоставляет для анализа данные приложения: метрики передаются в Prometheus, логи — в Loki, распределённые трассировки — в Tempo. Grafana запрашивает данные из этих хранилищ, отображает их на дашбордах и выполняет правила оповещений. Компоненты приложения не передают телеметрию в хранилища напрямую: основной поток проходит через Grafana Alloy.
...
| --- | --- |
| [`pyproject.toml`](pyproject.toml), [`uv.lock`](uv.lock) | Синхронное и Асинхронное Django-приложения, Фоновый обработчик заданий |
...
| [`rust-api/Cargo.toml`](rust-api/Cargo.toml) | Высокопроизводительный API |
...
| [`Dockerfile`](Dockerfile) | Общий образ Синхронного и Асинхронного Django-приложений и Фонового обработчика заданий |
| [`nginx.conf`](nginx.conf), [`nginx.deploy.conf.template`](nginx.deploy.conf.template) | Входной веб-шлюз и страница `/deploy-info/` |
| [`config.alloy`](config.alloy), [`docker-compose.dev-local.observability.yaml`](docker-compose.dev-local.observability.yaml), [`docker-compose.dev.observability.yaml`](docker-compose.dev.observability.yaml) | Коллектор наблюдаемости и его подключения к Платформе наблюдаемости |
