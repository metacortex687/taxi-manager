...
Отчёты формируются фоновым обработчиком вне цикла обработки пользовательского REST-запроса. Задания сохраняются в очереди на основе PostgreSQL с backend `django-tasks-db`. При росте нагрузки можно запускать дополнительные экземпляры фонового обработчика, горизонтально масштабируя выполнение длительных и ресурсоёмких операций. Результат отображается в веб-интерфейсе. Для отчёта **«Пробег автомобиля»** дополнительно доступно сохранение в формате PDF.
...
#### Примеры работы Grafana

Grafana Logs Drilldown позволяет отфильтровать журналы по сервису и перейти от распределения уровней логирования к конкретным записям Nginx и PostgreSQL, сохранённым в Loki.

![Grafana Logs Drilldown с журналами Nginx и PostgreSQL из Loki](docs/images/observability/grafana-logs-drilldown.png)

Grafana Traces Drilldown показывает частоту и длительность трассировок, ошибки и разбивку по сервисам. Из общего представления можно перейти к отдельной трассировке в Tempo и связать её с запросом приложения.

![Grafana Traces Drilldown с трассировками сервисов Taxi-manager из Tempo](docs/images/observability/grafana-traces-drilldown.png)
