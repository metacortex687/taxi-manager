...
Архитектура приложения описана набором взаимодополняющих представлений C4. Схемы разделены по назначению: основной функциональный контур, подключаемая подсистема уведомлений менеджеров, стек наблюдаемости и CI/CD-развёртывание.
...
Встроенный контейнер «Коллектор наблюдаемости» на Grafana Alloy собирает метрики, распределённые трассировки и логи приложения и передаёт их в Платформу наблюдаемости. Платформа хранит и визуализирует полученные данные, а также формирует оповещения.

Специалист сопровождения, CI/CD-контур и подключаемая подсистема уведомлений менеджеров вынесены в отдельные представления. Внутренние контейнеры и варианты физического развёртывания на контекстной диаграмме C1 не показываются.
...
| **Коллектор наблюдаемости** | Grafana Alloy собирает метрики, логи и трассировки контейнеров приложения и передаёт их в Платформу наблюдаемости |
...
Коллектор наблюдаемости Grafana Alloy является внутренним контейнером приложения и единой точкой сбора данных наблюдаемости. Он принимает метрики и трассировки приложений, собирает журналы контейнеров и передаёт телеметрию во внешнюю Платформу наблюдаемости.
