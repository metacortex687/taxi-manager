...
После подключения агента Jenkins-контроллер назначает ему этапы основного pipeline. Агент клонирует репозиторий Taxi-manager, получает базовые и тестовые образы, собирает приложение, запускает проверки и тесты, а после их успешного завершения обновляет Compose-проект `taxi-manager-deploy`. Контроллер также может использовать трассировки Tempo для поиска возможных N+1-запросов.
