...
Инструментация OpenTelemetry создаёт отдельные `span` для SQL-запросов, выполняемых Django. Поэтому обращения к PostgreSQL отображаются в Tempo отдельно внутри общей трассировки пользовательского запроса и связаны с ней общим `trace_id`. Сформированные приложением трассировки передаются в Tempo через Grafana Alloy.
...
Сотрудник автопарка выбирает автомобиль на странице онлайн-отслеживания React-приложения, после чего браузер открывает SSE-соединение через Nginx с Django ASGI. Django ASGI получает новые точки автомобиля из PostgreSQL через PgBouncer и передаёт их React-приложению в виде SSE-событий. Страница обновляет положение автомобиля на карте.

Сами точки поступают от Клиента телеметрии: Nginx направляет REST-запросы в Django WSGI, где данные сохраняются в PostgreSQL.

![Онлайн-отслеживание автомобиля через SSE](docs/images/c4/TaxiManagerLiveTrackingFlow.png)

#### Фоновое обратное геокодирование

При запросе поездок Django WSGI сначала ищет в PostgreSQL сохранённые адреса начальной и конечной точек. Если адрес не найден, приложение сохраняет задание в очереди на основе базы данных. Фоновый обработчик получает задание и повторно проверяет наличие адреса. Только при его отсутствии обработчик обращается к LocationIQ, после чего сохраняет в PostgreSQL адрес и соответствующую ему географическую область для повторного использования.
...
<details>
<summary><strong>3. Наблюдаемость приложения Taxi-manager</strong></summary>
...
Первоначально GitHub Actions использовался и для развёртывания самого приложения: workflow запускал тесты, собирал и публиковал Docker-образы, передавал конфигурацию на сервер по SSH и обновлял Docker Compose. После переноса основного конвейера CI/CD в Jenkins за GitHub Actions осталось развёртывание и обновление постоянного Jenkins-агента.
...
| --- | --- |
| [`pyproject.toml`](pyproject.toml), [`uv.lock`](uv.lock) | Django WSGI, Django ASGI и Фоновый обработчик заданий |
...
| [`Dockerfile`](Dockerfile) | Общий образ Django WSGI, Django ASGI и Фонового обработчика заданий |
...
GitHub Actions не разворачивает приложение непосредственно: он используется для доставки, запуска и обновления Jenkins-агента. Jenkins-мастер координирует основной конвейер CI/CD приложения, а Jenkins-агент выполняет его этапы.
