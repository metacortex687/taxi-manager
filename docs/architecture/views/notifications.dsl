systemContext notificationSystem "NotificationSystemContext" {
    ...
    description "Подсистема уведомлений менеджеров получает изменения основного приложения через CDC, авторизует пользователей через REST API и отправляет сообщения во внешний Чат уведомлений."
...
container notificationSystem "NotificationContainers" {
    ...
    description "База данных основного приложения является внешним источником CDC. Debezium публикует исходные события таблиц в Kafka, Flink преобразует их и возвращает в обработанные топики того же кластера. Сервис уведомлений самостоятельно читает обработанные топики, обновляет локальный контекст организаций и привязок и отправляет сообщения в Чат уведомлений."
