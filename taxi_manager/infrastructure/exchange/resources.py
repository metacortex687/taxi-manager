...
from taxi_manager.infrastructure.vehicle.models import Model, Vehicle
...
class ExchangeUuidResource(resources.ModelResource):
    ...
    def get_queryset(self):
        ...
        return (
            super()
            .get_queryset()
            .annotate(exchange_uuid=models.Subquery(uuid_subquery))
        )
    ...
    def _ensure_exchange_uuids_exist(self):
        ExchangeItem.objects.bulk_create(
            [
                ExchangeItem(
                    content_type=self._get_content_type(), object_id=enterprise.pk
                )
...
class EnterpriseResource(ExchangeUuidResource):
    time_zone = fields.Field(
        ...
        widget=widgets.ForeignKeyWidget(TimeZone, "code"),
    )
...
class ForeignUuidKeyWidget(widgets.ForeignKeyWidget):
    def render(self, value, obj=None, **kwargs):
        ...
        exchange_item = ExchangeItem.objects.filter(
            content_type=self._get_content_type(), object_id=value.id
        ).first()
        if exchange_item:
    ...
    def clean(self, value, row=None, **kwargs):
        exchange_item = ExchangeItem.objects.filter(
            content_type=self._get_content_type(), uuid=value
        ).first()
        if exchange_item:
...
class VehicleResource(ExchangeUuidResource):
    model = fields.Field(
        attribute="model", column_name="model", widget=ForeignUuidKeyWidget(Model, "id")
    )
    ...
    class Meta:
        ...
        fields = (
            ...
            "model",
            "enterprise",
        )  # enterprise пока нет возможности внешний ключ по uuid
        import_id_fields = ("exchange_uuid",)
...
class VehicleLocationResource(resources.ModelResource):
    ...
    def export_data_for_enterprise_and_period(self, enterprise, period_from, period_to):
        return self.export(
            ...
            .filter_period(period_from, period_to)
            .filter_enterprise(enterprise)
        )

    def clear_and_import_data_for_enterprise_and_period(
        self, dataset, enterprise, period_from, period_to, **kwargs
    ):
        VehicleLocation.objects.all().filter_period(
            period_from, period_to
        ).filter_enterprise(enterprise).delete()
...
class TripResource(ExchangeUuidResource):
    ...
    def clear_and_import_data_for_enterprise_and_period(
        ...
        trips_to_delete = Trip.objects.filter_period(
            period_from, period_to
        ).filter_enterprise(enterprise)
